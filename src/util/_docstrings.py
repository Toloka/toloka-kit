...
from typing import Any, Dict, List, TypeVar, Type, Collection
...
def inherit_docstrings(cls: Type[T]) -> Type[T]:
    ...
    cls.__doc__ = _construct_docstring(cls_docstring)
    for member_name in nested_docstrings:
        if nested_inherited_params[member_name]:
            ...
            getattr(cls, member_name).__doc__ = _construct_docstring(nested_docstrings[member_name])
...
def _construct_docstring(parsed_docstring: docstring_parser.Docstring) -> str:
    chunks = []
    docstring_prefix = ''
